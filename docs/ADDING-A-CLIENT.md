...
(`kimi`). Most steps are test-enforced — run the step-12 test set after each
...
1. **Upstream research first — before any code.** The official hooks/MCP docs
   ...
   Capture a live hook stdin payload and the MCP `initialize` `clientInfo.name`; it must begin
   with the same canonical client name used everywhere else. Decide now:
...
2. **`rust/crates/ds-client/src/lib.rs`** — variant, one canonical `as_str` value,
   `CLIENTS`, and the enumerating tests. That value is also the executable, launcher command,
   hook/IPC/log/usage token, and MCP-name prefix; do not add parallel names or aliases.
...
4. **`rust/crates/ds-config/src/wire/registry.rs`** — `CLIENT_REGISTRY` entry in
   `CLIENTS` order (order is test-pinned), `LaunchSpec` mode, a row in the observed
   MCP-name test,
...
6. **Launcher** — dispatch, usage text, and the unknown-subcommand hint are generated from
   `CLIENT_REGISTRY`; the new canonical client identity is the only accepted command.
...
10. **Docs** — [CLIENT-INTEGRATIONS.md](CLIENT-INTEGRATIONS.md) (supported list,
    Launch block, capability matrix, wiring table, `*_HOME` list,
