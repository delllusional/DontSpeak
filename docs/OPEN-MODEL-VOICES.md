...
Agent sticky map on main is `(Option<WiredAgent>, language) → voice id` in
`ttsq` — **agent-scoped**, not terminal-session-scoped. Wired agents use
`Some(agent)`; unattributed requests share one `None` bucket. SessionEnd barges
FIFO but keeps the assignment.
...
| --- | --- | --- |
| **Agent sticky** | `(Option<WiredAgent>, language) → voice id` | Same map; wired agents use `Some(agent)`, unattributed requests share one `None` bucket, and ids are **package ids**. `manage_voices select` writes it deliberately. |
