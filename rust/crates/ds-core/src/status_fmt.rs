...
///
/// `precision` is caller-supplied over the FFI boundary ([`crate::ffi::ds_stats_range`]) and is
/// clamped here: `format!("{:.precision$}")` panics for `precision >= 65536`, which would
/// otherwise be a concrete, reachable panic trigger for any FFI caller passing a large value —
/// deliberately or by a caller-side bug. No real stat display needs more than a few decimals.
