...
pub fn run(args: &[String]) -> i32 {
    ...
    for a in args {
        match a.as_str() {
            ...
            other => match WiredAgent::parse(other) {
                Some(t) => {
    ...
    if all {
        return wire_selected(WiredAgent::ALL, &paths, remove, print_only);
    }
...
fn wire_client(client: WiredAgent, paths: &Paths, remove: bool, print_only: bool) -> i32 {
    ...
    if client == WiredAgent::Codex && !remove && !print_only && code == 0 {
...
mod tests {
    ...
    fn wire_client_skips_absent_client() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        assert_eq!(wire_client(WiredAgent::Codex, &paths, false, false), 0);
    ...
    fn wire_client_remove_with_no_existing_config_is_a_noop() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        assert_eq!(wire_client(WiredAgent::Codex, &paths, true, false), 0);
    ...
    fn wire_client_qwen_code_wires_hooks_and_mcp_into_one_file_then_removes_both() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        make_present(&paths, WiredAgent::QwenCode);

        assert_eq!(wire_client(WiredAgent::QwenCode, &paths, false, false), 0);
        ...
        assert_eq!(wire_client(WiredAgent::QwenCode, &paths, true, false), 0);
    ...
    fn wire_client_codex_wires_hooks_and_mcp_into_one_file_then_removes_both() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        make_present(&paths, WiredAgent::Codex);
    ...
    fn wire_surfaces_print_only_codex_shows_the_union_of_both_surfaces() {
        ...
        assert_eq!(wire_client(WiredAgent::Codex, &paths, false, true), 0);
        assert!(
    ...
    fn wire_surfaces_print_only_qwen_shows_the_union_of_both_surfaces() {
        ...
        let spec = ds_config::client_spec(WiredAgent::QwenCode);
        let results = wire_surfaces_print_only(spec, &paths, false);
        assert_eq!(
            results.len(),
            1,
    ...
    fn wire_client_grok_wires_both_surfaces_then_removes_both() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        make_present(&paths, WiredAgent::Grok);

        assert_eq!(wire_client(WiredAgent::Grok, &paths, false, false), 0);
        ...
        assert_eq!(wire_client(WiredAgent::Grok, &paths, true, false), 0);
        assert!(
            !paths.grok_hooks_json.exists(),
    ...
    fn wire_client_kimi_code_wires_both_surfaces_then_removes_both_and_is_idempotent() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        make_present(&paths, WiredAgent::KimiCode);

        assert_eq!(wire_client(WiredAgent::KimiCode, &paths, false, false), 0);
        ...
        assert_eq!(wire_client(WiredAgent::KimiCode, &paths, true, false), 0);
    ...
    fn wire_client_hermes_wires_all_surfaces_then_removes_and_is_idempotent() {
        let dir = tempfile::tempdir().unwrap();
        let paths = Paths::rooted_at(dir.path());
        make_present(&paths, WiredAgent::Hermes);

        assert_eq!(wire_client(WiredAgent::Hermes, &paths, false, false), 0);
        ...
        assert_eq!(wire_client(WiredAgent::Hermes, &paths, true, false), 0);
    ...
    fn malformed_shared_configs_are_unchanged_hard_errors_for_every_surface() {
        ...
        make_present(&qwen, WiredAgent::QwenCode);
        ...
        assert_eq!(wire_client(WiredAgent::Codex, &codex, false, false), 1);
        ...
        make_present(&hermes, WiredAgent::Hermes);
        ...
        assert_eq!(wire_client(WiredAgent::Hermes, &hermes, false, false), 1);
    ...
    fn wire_surfaces_print_only_hermes_shows_hooks_and_mcp_union() {
        ...
        let spec = ds_config::client_spec(WiredAgent::Hermes);
        let results = wire_surfaces_print_only(spec, &paths, false);
    ...
    fn reconcile_strips_a_client_dropped_from_the_desired_set() {
        ...
        let cfg = ds_config::VoiceConfig {
            exclude_clients: Some(vec![WiredAgent::QwenCode]),
    ...
    fn manual_wire_selection_survives_boot_reconcile_in_both_directions() {
        ...
        assert_eq!(
            wire_selected(&[WiredAgent::QwenCode], &paths, false, false),
            0
        );
        assert_eq!(
            wire_selected(&[WiredAgent::QwenCode], &paths, true, false),
            0
        );
        assert_eq!(reconcile(&paths), 0);
