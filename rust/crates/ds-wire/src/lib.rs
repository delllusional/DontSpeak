...
fn wire_client(client: ClientSource, paths: &Paths, remove: bool, print_only: bool) -> i32 {
    ...
    // Codex-only user hint: wiring installs the contracts; the launcher coordinates the
    // app-server lifecycle needed for mid-turn narration. Keep one cross-platform command
    // here and the client-specific details in docs/CLIENT-INTEGRATIONS.md.
