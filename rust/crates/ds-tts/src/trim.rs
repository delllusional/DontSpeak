...
mod tests {
    ...
    fn short_signal_no_panic() {
        // Shorter than a single frame's worth of real samples; must not panic — and since a
        // uniform-amplitude input forms exactly one RMS frame that's its own 0 dB reference,
        // nothing crosses the trim threshold, so the whole (tiny) signal survives unchanged.
        let y = vec![0.5f32; 10];
        assert_eq!(trim_silence(&y), y);
    }
}
