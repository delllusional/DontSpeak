...
mod tests {
    ...
    fn voice_pack_path_under_ane_dir() {
        // `am_adam` always sanitizes to a non-empty id, so the only way this resolves to
        // `None` is `$HOME` being unset — `.expect` so that case fails loudly instead of the
        // assertion silently no-op'ing.
