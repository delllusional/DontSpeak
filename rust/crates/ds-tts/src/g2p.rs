...
mod tests {
    ...
    #[test]
    fn phonemize_never_panics_on_degenerate_input() {
        // Truly empty/whitespace-only input has no words to phonemize; just must not panic.
        for t in ["", "   "] {
            let _ = phonemize(t);
        }
    }
    ...
    #[test]
    fn phonemize_produces_mappable_output_for_punctuated_and_odd_input() {
        for t in [
            "Hello, world! How are you?",
            "Code: foo_bar();",
            "3.14 and 42",
            "zzqxq", // likely OOV; spelled out (never silent), so still non-empty output.
        ] {
            let ph = phonemize(t);
            assert!(!ph.trim().is_empty(), "expected phonemes for {t:?}");
            let ids = crate::vocab::tokenize(&ph);
            assert!(!ids.is_empty(), "tokenize should yield ids for {t:?}");
        }
    }
