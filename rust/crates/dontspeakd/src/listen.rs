...
impl TurnLogic {
    pub fn new(words: &ds_config::HandsFreePhrases, confirm_ms: u64) -> Self {
        Self {
            start_word: words.start.clone(),
            submit_word: words.submit.clone(),
            cancel_word: words.cancel.clone(),
            confirm_ms,
            capturing: false,
            buffer: String::new(),
            pending: None,
    ...
    pub fn on_speech_onset(&mut self) -> Vec<TurnAction> {
        if let Some(p) = self.pending.take() {
            let word = match p.kind {
                PendingKind::Submit => self.submit_word.clone(),
                PendingKind::Cancel => self.cancel_word.clone(),
            };
            self.append(&word);
        }
        vec![]
...
mod tests {
    ...
    #[test]
    fn cancel_with_leading_content_in_same_segment() {
        // Mirrors the submit-with-content-before coverage: content preceding the stop
        // word in the SAME segment must still be appended before arming the cancel.
        let mut t = turn();
        t.on_segment("computer draft the email");
        t.on_segment("wait actually cancel");
        assert_eq!(t.buffer(), "draft the email wait actually");
        assert_eq!(t.on_tick(1000), vec![TurnAction::Cancel]);
    }
