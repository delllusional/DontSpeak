...
/// value ΓÇö old triple fields could diverge).
...
pub(crate) enum FinalState {
    ...
    Ready(String),
    /// Empty final ΓÇö tick disarms without pasting; still awaiting until disarm.
    Empty,
}

/// Engine Γåö confirm-panel channel (`model_status.dictation`). Helper writes;
...
pub(crate) struct PasteBuf {
    ...
    pub can_paste: bool,
    /// Caps held ΓÇö suppress Ready (might become long-press cancel).
    pub caps_held: bool,
    ...
    /// Frontend-owned PTT: subscriber's app was frontmost at start → lifecycle +
    /// acked deliver (not paste); served `dictation.state` is `hidden`.
    pub frontend_owned: bool,
}

impl PasteBuf {
    /// `Idle ΓåÆ Armed`. No-op on Ready/Empty (joiner may deposit between stop and arm).
    fn arm(&mut self) {
    ...
    /// `Armed|Empty ΓåÆ Idle`; leave Ready (call sites take text first).
    fn disarm(&mut self) {
...
/// Frontend-facing dictation phase (Zed). Same snapshot as overlay
/// ([`Engine::publish_status_change`]); transitions → [`frontend_events`].
/// Precedence mirrors [`crate::status::dictation_state`] (awaiting > recording >
/// refused > hidden).
#[derive(Debug, Clone, PartialEq, Eq)]
enum FrontendPhase {
    /// Idle or not frontend-owned.
    Hidden,
    /// Live partial (may be empty right after start).
    Recording { text: String },
    /// Stop fired; confirm pending (partial until final lands).
    Awaiting { text: String },
    /// Frontend-owned start refused (engine not ready).
    Refused,
}

/// Phase from the tick snapshot. PURE. `owned` gates the surface; `defers_paste`
/// ≡ local STT (ClaudeNative has no partials/confirm → stays Hidden).
fn derive_frontend_phase(
    owned: bool,
    recording: bool,
    defers_paste: bool,
    awaiting: bool,
    ...
    if !owned {
        FrontendPhase::Hidden
    } else if awaiting {
        FrontendPhase::Awaiting {
            text: text.to_string(),
        }
    } else if recording && defers_paste {
        FrontendPhase::Recording {
            text: text.to_string(),
        }
    } else if refused {
        FrontendPhase::Refused
    } else {
        FrontendPhase::Hidden
    }
}

/// Events for a `prev → cur` phase transition. PURE. Live surface that vanishes (or
/// is replaced by a new recording) emits `cancelled` first so the frontend clears
/// marked text. Same-dictation progressions never do — all cancel/empty/teardown
/// paths hit `Hidden` here. Successful deliver pre-settles to `Hidden` in
/// `confirm_paste` so deliver stays terminal.
fn frontend_events(prev: &FrontendPhase, cur: &FrontendPhase) -> Vec<ds_ipc::DictationEvent> {
    use FrontendPhase as P;
    use ds_ipc::DictationEvent as E;
    if prev == cur {
        return Vec::new();
    }
    let mut out = Vec::new();
    let prev_live = matches!(prev, P::Recording { .. } | P::Awaiting { .. });
    let ended = match (prev, cur) {
        // Same-dictation progressions — nothing ended.
        (P::Recording { .. }, P::Awaiting { .. }) => false,
        (P::Recording { .. }, P::Recording { .. }) => false,
        (P::Awaiting { .. }, P::Awaiting { .. }) => false,
        // Reachable: new PTT `start_recording` while ConfirmArmed (wipes armed confirm).
        // Pinned by `frontend_events_transition_table` (Cancelled + RecordingStarted).
        (P::Awaiting { .. }, P::Recording { .. }) => true,
        // The surface vanished (cancel/empty final/teardown/deliver fallback).
        (_, P::Hidden | P::Refused) => prev_live,
        (P::Hidden | P::Refused, _) => false,
    };
    if ended {
        out.push(E::Cancelled);
    }
    match cur {
        P::Hidden => {}
        P::Refused => out.push(E::Refused),
        P::Recording { text } => {
            if matches!(prev, P::Recording { .. }) {
                // Still the same recording — only the live partial changed.
                out.push(E::Partial { text: text.clone() });
            } else {
                out.push(E::RecordingStarted);
                if !text.is_empty() {
                    out.push(E::Partial { text: text.clone() });
                }
            }
        }
        P::Awaiting { text } => out.push(E::AwaitingConfirm { text: text.clone() }),
    }
    out
}
...
pub(crate) enum GestureState {
    ...
    ConfirmArmed {
        ...
        /// Press began inside flip window ΓåÆ not a new recording.
        double_pending: bool,
...
enum PressState {
    Up,
    /// Latch so cancel fires once and release Γëá tap.
    Down {
        since: Instant,
...
pub(crate) struct Engine<P: Platform + 'static> {
    ...
    /// INVARIANT: every `self.gesture = ΓÇª` is followed by `sync_caps_led` (or a helper that does).
    ...
    /// Hold ΓëÑ this (ms) ΓåÆ force idle, LED off.
    long_press_ms: u64,
    press: PressState,
    ...
    /// Caps loop live; false ΓçÆ `tick` no-op.
    pub(crate) caps: bool,
    ...
    /// Live frontend subscriptions (`spawn_ipc_server`). `None` in tests → frontend paths no-op.
    pub(crate) frontends: Option<Arc<crate::ipc::FrontendRegistry>>,
    /// Last streamed [`FrontendPhase`] (transition anchor for [`frontend_events`]).
    frontend_phase: FrontendPhase,
    /// Overlay digest (bump only on change).
    dict_digest: u64,
    /// Local helper STT vs factory fallback ΓÇö reload rebuilds when this flips.
...
impl<P: Platform + 'static> Engine<P> {
    ...
    pub(crate) fn new(plat: P, long_press_ms: u64) -> Self {
        ...
        // Space chord ΓÇö real engine reads Claude Code binding via `with_config`/factory.
    ...
    fn assemble(
        ...
        let mut engine = Self {
            ...
            status_gate: None,
            frontends: None,
            frontend_phase: FrontendPhase::Hidden,
            dict_digest: 0,
    ...
    /// Sole `Platform::set_caps_lock` write in this file ΓÇö call last after every `gesture` assign.
    ...
    /// confirm, paste target, frontend ownership) ΓÇö the `recording`/`stt_active` flag
    /// itself is pushed at its flip site by [`set_stt_active`], so re-digesting it here
    /// would only double-bump. Skips the bump when unchanged so an idle engine never
    /// wakes waiters every tick. No-op in tests (`status_gate` is `None`).
    ///
    /// ALSO the single frontend-event emission site: the same snapshot feeds
    /// [`Self::emit_frontend_events`], so a subscribed native frontend's event stream is
    /// derived from the SAME state transition that wakes the overlay ΓÇö every
    /// dictation-ending path automatically surfaces as a transition to hidden (ΓçÆ
    /// `cancelled`), with no per-path hand-placed emits to forget.
    fn publish_status_change(&mut self) {
        use std::hash::{Hash, Hasher};
        // Nothing to publish to: no push gate (tests) AND no frontend registry.
        if self.status_gate.is_none() && self.frontends.is_none() {
            return;
        }
        let (text, awaiting, has_target, refused, frontend_owned) = self
            .paste
            .lock()
            .map(|p| {
                ...
                // pop-up and its fade-out each get a push within one tick ΓÇö no separate
                // expiry bookkeeping.
                (
                    t,
                    a,
        ...
        self.emit_frontend_events(frontend_owned, awaiting, &text, refused);
        ...
        refused.hash(&mut h);
        // Frontend ownership flips the SERVED `dictation.state` token (the overlay
        // suppression override in `status::served_dictation_state`), so its edges need
        // a push too.
        frontend_owned.hash(&mut h);
        let digest = h.finish();
    ...
    /// One emission site for frontend lifecycle events (from
    /// [`Self::publish_status_change`]). Clears ownership if all subscribers drop.
    fn emit_frontend_events(&mut self, owned: bool, awaiting: bool, text: &str, refused: bool) {
        let Some(reg) = self.frontends.clone() else {
            return;
        };
        let cur = derive_frontend_phase(
            owned,
            self.is_recording(),
            self.stt.defers_paste(),
            awaiting,
            text,
            refused,
        );
        if cur == self.frontend_phase {
            return;
        }
        for event in frontend_events(&self.frontend_phase, &cur) {
            if !reg.broadcast(event) {
                if owned && let Ok(mut p) = self.paste.lock() {
                    p.frontend_owned = false;
                }
                // Settle locally; a later subscriber must not inherit this flow.
                self.frontend_phase = FrontendPhase::Hidden;
                return;
            }
        }
        self.frontend_phase = cur;
    }

    /// Next PTT is frontend-owned? Live sub + app frontmost. Decided once at
    /// [`Self::start_recording`] (confirm re-checks frontmost for deliver/paste).
    fn frontend_owns_next_dictation(&self) -> bool {
        let Some(reg) = &self.frontends else {
            return false;
        };
        let plat = &self.plat;
        reg.any_subscriber_frontmost(&|app| plat.is_app_frontmost(app))
    }

    /// Acked deliver to frontmost sub → skip paste; else false (paste fallback).
    fn deliver_to_frontend(&self, text: &str, submit: bool) -> bool {
        let Some(reg) = &self.frontends else {
            return false;
        };
        let plat = &self.plat;
        reg.deliver_to_frontmost(text, submit, &|app| plat.is_app_frontmost(app))
            == crate::ipc::DeliverOutcome::Delivered
    }
    ...
    /// Internal end of in-flight dictation (reload / caps-gate off) ΓÇö aborts listen, resets
    ...
    fn teardown_hold(&mut self) {
        ...
        // Internal end still ends recording ΓÇö LED off even if no physical release is coming.
        self.sync_caps_led();
    ...
    /// `ConfirmArmed ΓåÆ Idle` (drops flip sub-state) + disarms paste mirror. No-op on
    ...
    /// Any non-`Idle` final (incl. Armed) ΓÇö gates tick's paste-target probe while panel is up.
    ...
    /// text field ΓÇö ANY app, the synthetic Cmd+V lands wherever the cursor is ΓÇö then press
    /// Return whenever `enter_after_paste` is set (armed per `double_tap_submits` and the
    /// stop gesture's tap count ΓÇö one of the two gestures always submits, the other always
    /// just inserts).
    ...
    fn confirm_paste(&mut self) {
        ...
        // The confirm tap ALWAYS pastes ΓÇö there's no focus refusal. The "is there a
        // paste target?" cue is a live glow on the panel (the engine samples
        // `can_paste` (the platform probe) each tick ΓåÆ `can_paste` (the
        // status field) ΓåÆ the app tints it orange when there's nowhere to land).
        //
        // The frontend-owned tag is TAKEN under the same lock: ownership was decided at
        // `start_recording`, and consuming it here means the served overlay token
        // reverts no matter how the delivery below resolves (a failed deliver falls
        // back to the classic paste with classic status).
        let (text, frontend_owned) = self
            .paste
            .lock()
            .ok()
            .map(|mut p| {
                p.partial.clear();
                p.target = None;
                let owned = std::mem::replace(&mut p.frontend_owned, false);
                // Take the text and drop to `Idle` atomically under this ONE lock ΓÇö not
                ...
                let text = match std::mem::replace(&mut p.final_state, FinalState::Idle) {
        ...
        if let Some(text) = text {
            ...
            // ANY focused app ΓÇö terminal, chat box, search field, editor); the other always
            ...
            // Frontend-owned dictation: deliver over the subscription socket (acked,
            // bounded). Delivered ΓçÆ the frontend inserted the text and dispatches its
            // own Enter when submitting ΓÇö skip `type_text`/`press_enter` entirely.
            // Failed (no frontmost subscriber anymore, write error, nack, timeout) ΓçÆ
            // fall through to the unchanged paste path below; nothing is ever lost.
            delivered = frontend_owned && self.deliver_to_frontend(&text, submit);
            if delivered {
                if submit && let Some(q) = &self.ttsq {
                    // Submit-bookkeeping PARITY with the Enter path below: apply
                    // `clear_on_input` at submit time exactly as a pasted submit would
                    // (same scope resolution ΓÇö see the paste branch's comment), and
                    // note the voice submit NOW so the `UserPromptSubmit ΓåÆ MarkActive`
                    // fired when the frontend's own Enter lands is deduplicated as
                    // THIS submit's echo, not counted as a separate typed one
                    // (`note_voice_submit`'s de-dup window is seconds wide, so
                    // stamping it at deliver time instead of the frontend's keystroke
                    ...
                    // Deliver: note now (frontend owns Enter). Paste branch notes in
                    // `press_deferred_enter` instead ΓÇö engine-driven Enter de-dup.
                    q.note_voice_submit();
                }
                // A successful deliver is TERMINAL for the frontend flow: pre-settle
                // the emitter so the awaitingΓåÆhidden transition the next tick observes
                // doesn't also stream a trailing `cancelled` after the deliver.
                self.frontend_phase = FrontendPhase::Hidden;
            } else {
                // Paste into WHATEVER is focused (terminal, Notes, browser, chat, ΓÇª). The
                // explicit confirm tap + the overlay's "ΓåÆ <app>" target label are the
                // deliberate gate now, so the paste is no longer restricted to a terminal.
                self.plat.type_text(&text);
                if submit {
                    if let Some(q) = &self.ttsq {
                        // Apply `clear_on_input` immediately at submit ΓÇö must not wait on the
                        ...
                        // Deliberately no `note_voice_submit` here ΓÇö that is pinned to the
                        // real Enter in `press_deferred_enter` (see deliver branch above).
                        q.cancel_for_submit(
                            q.active_session(),
                            self.cfg.clear_on_input.contains(&CancelSpeechScope::Current),
                            self.cfg.clear_on_input.contains(&CancelSpeechScope::Other),
                    ...
                    // Let the async paste settle before Enter lands ΓÇö deferred via a polled
        ...
        if delivered {
            log::debug!(
                target: "engine",
                "deferred submit ΓÇö delivered transcript to the subscribed frontend, LED off"
            );
        } else if inserted_only {
            log::debug!(
                target: "engine",
                "deferred submit ΓÇö pasted pending transcript (insert only, no Enter), LED off"
            );
        } else {
            log::debug!(target: "engine", "deferred submit ΓÇö pasted pending transcript + Enter, LED off");
        }
    }
    ...
    fn cancel_all(&mut self) {
        ...
        // Hold means "stop talking" ΓÇö clear queue, no resume.
        if let Some(q) = &self.ttsq {
            q.clear();
        }
        self.gesture = GestureState::Idle;
        ...
        log::debug!(target: "engine", "HOLD cancel ΓÇö dictation discarded, voice silenced, LED off, idle");
    }
    ...
    fn stt_ready_to_dictate(&self) -> bool {
        ...
        // Tests / pure-RPC: no warm host ΓÇö on/off only.
    ...
    fn toggle_dictation(&mut self) {
        ...
        // with nothing to resume). Already recording ΓåÆ always stop.
    ...
    pub(crate) fn reload(&mut self, cfg: &VoiceConfig) {
        ...
        // `resolved_stt` stays `built_in` but `build_stt` would switch ClaudeNative ΓåÆ helper).
        // Gate on `tts.is_some()` to mirror `build_stt` (tests: no helper ΓçÆ factory only).
        let want_local = self.tts.is_some() && local_stt_available(cfg);
        // Capture before overwrite ΓÇö listener lifecycle uses the same edge.
        ...
        // Every reload: pick up Accessibility grants; OFF mid-hold ΓåÆ teardown_hold via gate.
        ...
        if let Some(tts) = &self.tts {
            ...
            // Seed provider before first `set_enabled` ΓÇö autofetch applies resolved EP only
            // after `daemon.reload` returns (`boot::engine_run` / ReloadTick). Same as boot.
            tts.set_provider(cfg.resolved_tts_provider().as_str());
        }

        // Helper hosts TTS+STT ΓÇö re-gate when either engine needs change.
        ...
        if cfg.listen_mode == ds_config::ListenMode::Always {
            // Listener freezes availability + provider at construction ΓÇö rebuild on
            ...
            if self.listener.is_none()
                ...
                || change.stt_changed
            {
                // Hands-free bypasses Caps PTT ΓÇö tear down in-flight / deferred submit.
            ...
            // Listener drives shared stt_active + paste ΓÇö teardown before drop.
            self.teardown_hold();
            self.listener = None;
        }
        ...
        self.cfg = cfg.clone();
        // `press` is physical-key latch ΓÇö leave across reload.
        log::info!(
            target: "engine",
    ...
    /// (the app's dot) sees it immediately ΓÇö mirrors [`set_stt_active`]. Without
    ...
    /// native OS behavior instead of leaving the key suppressed-but-ignored ΓÇö and so
    ...
    fn set_caps_gate(&mut self, now_on: bool) {
        ...
        if now_on != self.caps {
            if now_on {
                ...
                // to reset (Windows' `release_caps_key` wipes its edge queue) ΓÇö any
                ...
                // cheap, correct no-op write) ΓÇö belt-and-suspenders, and harmless
    ...
    /// since last time, flip the caps loop on/off live ΓÇö so GRANTING Accessibility
    ...
    /// [`ds_platform::CapsKeyMonitor::caps_monitor_stuck`]) ΓÇö the caller
    ...
    /// Which resource `needs_relaunch()` is `true` because of ΓÇö see
    ...
    /// draining it. No-op on polled platforms (`is_caps_event_driven` false) ΓÇö they have
    ...
    /// physically OWNED ΓÇö and so still queuing ΓÇö while either one alone would suggest
    ...
    ///   * a LONG-PRESS (hold ΓëÑ `long_press_ms`) force-resets to idle;
    ///   * the Caps LED is a pure OUTPUT derived from `is_recording()` via
    ///     `sync_caps_led()`, called at every point `self.gesture` changes ΓÇö never
    ...
    pub(crate) fn tick(&mut self) {
        ...
        // must still fire even if caps gets disabled or the mode changes mid-delay ΓÇö
        ...
        // affine), so the poll thread samples it here ΓÇö every tick, before any mode
        ...
        if let Some(q) = &self.ttsq {
            ...
            // When it's off, the frontmost probe is dead ΓÇö so skip the poll/main-thread
            // NSWorkspace round-trip (~33├ù/s forever in the common idle case) and publish
        ...
        // tint the glow when there's nowhere to paste. Only while the panel shows ΓÇö the
        ...
        if recording || self.awaiting_confirm() {
            // A focused editable field is the primary signal, but terminals ΓÇö the app's
            // MAIN dictation target ΓÇö frequently don't expose an AX-settable text element
            ...
            // (`listener::exec`) paths paste unconditionally into whatever is focused ΓÇö
            // there's no focus refusal ΓÇö so the glow must not warn there.
        ...
        // `WaitModelStatus` so the app re-renders the overlay immediately (Γëñ one tick of
        // latency) instead of waiting out its status-poll timer. Runs in every mode
        // (PTT, always-listen, caps-off) ΓÇö it only bumps on an actual change. Recording
        ...
        if self.cfg.listen_mode == ds_config::ListenMode::Always {
            ...
            // platform's key ownership) is a SEPARATE axis ΓÇö an event-driven platform
        ...
        // polling and no emits ΓÇö it's a pure RPC host for the other subsystems.
        if !self.caps {
            // Same reasoning as above ΓÇö belt-and-suspenders in case the platform's own
        ...
        // GESTURE MODEL ΓÇö "tap to dictate, hold to cancel". Driven entirely off the
        // PHYSICAL Caps key (down / hold / up), NOT the OS lock latch:
        //   ΓÇó DOWN    ΓÇö nothing yet. Start the press timer; re-assert the LED to the
        //               real recording state so the OS's own latch-flip never changes
        //               the light on a press.
        //   ΓÇó HOLD ΓëÑ long_press_ms ΓÇö CANCEL: discard any in-flight dictation AND silence
        //               the voice/generation, back to idle. Never records, never lights.
        //   ΓÇó quick UP (released before the threshold) ΓÇö a TAP toggles dictation: start
        //               when idle, stop+submit when recording, via `start_recording`/
        //               `stop_recording` ΓÇö which sync the LED themselves. For an
        ...
        // The Caps LED is a pure OUTPUT derived from `is_recording()` ΓÇö every function
        ...
        // for the ~30 ms poll to even observe the key-down is missed ΓÇö tap again; the
        ...
        //   ΓÇó EVENT-DRIVEN (Windows low-level hook) ΓÇö drain the lossless queue and replay
        ...
        //   ΓÇó POLLED (macOS / Linux / tests) ΓÇö sample the held boolean and synthesize one
        ...
        // The LED is already OFF (driven on the stop tap's release) ΓÇö this only moves text.
        ...
        if self.is_confirm_armed() && !self.deferred_submit_held() {
            ...
            if has_ready {
                ...
                // The deferred final landed EMPTY ΓÇö nothing to submit. Disarm
                // (`disarm_confirm` maps the buffer's `Empty ΓåÆ Idle` too).
                self.disarm_confirm();
                self.record_caps("confirm");
            }
    ...
    ///   * ANY Caps press in flight while armed ΓÇö the press resolves to the double's
    ...
    ///     has landed yet. Unconditionally honored ΓÇö exactly one of the two stop-tap
    ...
    /// double tap must still be waited for to flip it to `true` ΓÇö gating on the
    ...
    ///   * DOWN ΓÇö begin a press; the decision defers to the release (tap) or the
    ///     long-press threshold (hold), both handled in [`check_long_press`].
    ///   * UP ΓÇö a release NOT consumed by a long-press is a TAP ΓåÆ toggle dictation
    ...
    fn apply_caps_edge(&mut self, down: bool, at: Instant) {
        ...
        if down {
            ...
            // tap of the flip double (or a long-press cancel of the pending paste) ΓÇö
            ...
            // still-unpasted transcript. Unconditional ΓÇö no config gates this: consuming
            ...
            // `sync_caps_led()` call ΓÇö never here directly. For an IMMEDIATE tap (not
            ...
            // release edge left by then ΓÇö which is exactly why the write had to move
            // into `start_recording`/`stop_recording` themselves rather than staying
            // here. `Up ΓçÆ tap` matches the old `!self.long_press_fired` (which read
            // true even with no press latched ΓÇö e.g. a release edge observed after a
            // gate offΓåÆon cycle reset the latch but not `caps_phys_prev`).
            let was_tap = matches!(
                self.press,
                PressState::Down {
                    long_press_fired: false,
    ...
    /// IMMEDIATELY ΓÇö starting dictation from silence keeps zero added latency.
    fn handle_tap(&mut self, at: Instant) {
        // Second tap of the stop double ΓåÆ FLIP the armed outcome (see
        // `enter_after_paste`). Checked before the speak-skip logic ΓÇö the stop tap
            ...
            && *double_pending
        {
            ...
            *stop_tap_at = None;
            ...
            if submits {
                log::debug!(target: "engine", "double-tap on stop ΓÇö will submit (paste + auto-Enter)");
            } else {
                log::debug!(target: "engine", "double-tap on stop ΓÇö insert only (paste, no auto-Enter)");
            }
            return;
        }
        ...
        match tap_decision(speaking, self.pending_tap_at, at, window) {
            // Not speaking (or no prior tap to pair) ΓÇö act now, no added latency.
            TapAction::Immediate => {
                self.pending_tap_at = None;
                self.toggle_dictation();
            }
            // Second tap inside the window ΓåÆ DOUBLE-TAP: skip the current message.
            TapAction::Skip => {
                self.pending_tap_at = None;
                if let Some(q) = &self.ttsq {
                    q.skip_current();
                }
                log::debug!(target: "engine", "double-tap ΓÇö skipped current message, advancing to next");
            }
            // First tap while speaking ΓåÆ defer; the single fires from `tick` if no
    ...
    /// second tap. Skipped while a Caps press is in flight (`press` is `Down`) ΓÇö that
    /// could be the second tap of a double, or a hold becoming a long-press ΓÇö so the single
    ...
    /// a Caps hold past `long_press_ms` force-resets to idle (CANCEL ΓÇö discard
    ...
    /// OS's own hold-delay latch flip on the polled ports ΓÇö a no-op on Windows, which
    ...
    /// Start dictation ΓÇö called from `toggle_dictation`, either immediately (a tap's
    ...
    /// lapses with no second tap ΓÇö see `handle_tap`'s doc). No-op if already
    ...
    fn start_recording(&mut self) {
        ...
        // path), so this guard is only reached defensively ΓÇö just don't record.
        ...
        // safe in full-duplex ΓÇö the open VPIO mic stays live for the dictation.
        ...
        // speaking-deferred tap's double-tap window lapses ΓÇö in the deferred case
        ...
        // land. We never steal focus here ΓÇö the transcript pastes into whatever the
        // user is in (the focus-gated Ctrl+G / paste targets the current frontmost app).
        let target = self.plat.frontmost_app_name();
        let frontend_owned = self.frontend_owns_next_dictation();
        if let Ok(mut p) = self.paste.lock() {
            p.partial.clear();
            p.final_state = FinalState::Idle;
            p.target = target;
            p.frontend_owned = frontend_owned;
        ...
        // never observe our mic as foreign in the gap before the flag ΓÇö the gate is
        ...
        log::debug!(target: "engine", "LED ON ΓÇö stt.start()");
    }

    /// Stop dictation (full-mirror ONΓåÆOFF edge). No-op if not recording. Ends the
    ...
    fn stop_recording(&mut self) {
        ...
        // The STOPPING press IS the submit gesture in BOTH modes ΓÇö a quick release
        // submits, a HELD press cancels (the long-press ΓåÆ discard/reset). There is no
        ...
        if self.stt.defers_paste() {
            ...
            // `FinalState::Armed`) ΓÇö keeps the panel up across the gap between this
            // synchronous `recording ΓåÆ false` flip and the async final landing.
        ...
        log::debug!(target: "engine", "LED OFF ΓÇö stt.stop()");
    }
...
/// What a completed Caps TAP does ΓÇö UNIFIED across dictation on/off so the gesture means
...
/// case ([`Refused`](CapsTap::Refused)) ΓÇö neither a record toggle nor a voice pause.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CapsTap {
    /// Dictation on + ready, idle ΓåÆ begin recording (which pauses the voice).
    StartRecord,
    /// Dictation on, recording ΓåÆ stop + submit (which resumes the voice).
    StopRecord,
    /// Dictation on but NOT READY (model still downloading/loading), idle ΓåÆ refuse the
    /// start: a strict no-op for the voice (the caller arms the visual refusal cue).
    /// Deliberately NOT the OFF pause/resume gesture ΓÇö a pause here has no matching
    /// resume when the model comes ready, so it silenced TTS for the whole download
    /// window (issue #1).
    Refused,
    /// Dictation off, voice playing/idle ΓåÆ pause the voice (hold; nothing dropped).
    PauseVoice,
    /// Dictation off, voice paused ΓåÆ resume the voice.
    ResumeVoice,
}
...
/// what made on-but-not-ready borrow the OFF pause/resume path (issue #1). Pure ΓÇö the
...
/// What a Caps tap should do ΓÇö the pure time-and-state core of [`Engine::handle_tap`].
#[derive(Debug, PartialEq, Eq)]
enum TapAction {
    /// Act now (the normal toggle). Not speaking, or a stale prior tap ΓåÆ treat as a new one.
    Immediate,
    /// First tap while speaking ΓÇö hold the action until the double-tap window lapses.
    Defer,
    /// Second tap within the window ΓåÆ skip the current message.
    Skip,
}

/// Whether `t0` (if any) is still within [`DOUBLE_TAP_MS`] of `now` ΓÇö the "recent tap"
...
/// Not speaking ΓçÆ `Immediate` (zero added latency on starting dictation from silence).
/// Speaking ΓçÆ `Skip` if a prior tap is within `window`, else `Defer` (incl. a stale prior
/// tap, which the caller has already fired from `tick`). Pure ΓÇö exhaustively unit-tested.
fn tap_decision(
    speaking: bool,
...
#[cfg(test)]
mod tests {
    // ── Frontend phase pure tables (Zed) ─────────────────────────────
    #[test]
    fn frontend_events_transition_table() {
        use ds_ipc::DictationEvent as E;
        use FrontendPhase as P;
        let rec = |t: &str| P::Recording { text: t.into() };
        let awa = |t: &str| P::Awaiting { text: t.into() };

        // No transition ΓçÆ nothing.
        assert!(frontend_events(&P::Hidden, &P::Hidden).is_empty());
        assert!(frontend_events(&rec("a"), &rec("a")).is_empty());

        // Start: empty text announces only the start; a non-empty first observation
        // (e.g. a partial landing within the same tick) announces both.
        assert_eq!(
            frontend_events(&P::Hidden, &rec("")),
            vec![E::RecordingStarted]
        );
        assert_eq!(
            frontend_events(&P::Hidden, &rec("hi")),
            vec![E::RecordingStarted, E::Partial { text: "hi".into() }]
        );

        // Partial growth.
        assert_eq!(
            frontend_events(&rec("h"), &rec("hi")),
            vec![E::Partial { text: "hi".into() }]
        );

        // Stop tap: same dictation ΓÇö no cancelled.
        assert_eq!(
            frontend_events(&rec("hi"), &awa("hi")),
            vec![E::AwaitingConfirm { text: "hi".into() }]
        );
        // The async final replacing the partial re-announces the confirm text.
        assert_eq!(
            frontend_events(&awa("hi"), &awa("hi there")),
            vec![E::AwaitingConfirm {
                text: "hi there".into()
            }]
        );

        // A NEW recording replacing an armed confirm wiped the pending transcript.
        assert_eq!(
            frontend_events(&awa("hi"), &rec("")),
            vec![E::Cancelled, E::RecordingStarted]
        );

        // Vanishing surfaces cancel; refusals from nothing don't.
        assert_eq!(frontend_events(&rec("hi"), &P::Hidden), vec![E::Cancelled]);
        assert_eq!(frontend_events(&awa("hi"), &P::Hidden), vec![E::Cancelled]);
        assert_eq!(frontend_events(&P::Hidden, &P::Refused), vec![E::Refused]);
        assert!(frontend_events(&P::Refused, &P::Hidden).is_empty());
    }

    #[test]
    fn derive_frontend_phase_mirrors_the_dictation_state_precedence() {
        use FrontendPhase as P;
        // Not owned ΓçÆ Hidden, whatever the surface state says.
        assert_eq!(
            derive_frontend_phase(false, true, true, true, "t", true),
            P::Hidden
        );
        // Awaiting wins over everything (the finalized transcript must show).
        assert_eq!(
            derive_frontend_phase(true, true, true, true, "t", true),
            P::Awaiting { text: "t".into() }
        );
        // Recording needs a deferred-paste engine (ClaudeNative surfaces nothing ΓÇö
        // same suppression as the overlay's `local_stt` gate).
        assert_eq!(
            derive_frontend_phase(true, true, true, false, "t", false),
            P::Recording { text: "t".into() }
        );
        assert_eq!(
            derive_frontend_phase(true, true, false, false, "t", false),
            P::Hidden
        );
        // Refused sits below recording, above hidden.
        assert_eq!(
            derive_frontend_phase(true, false, true, false, "", true),
            P::Refused
        );
        assert_eq!(
            derive_frontend_phase(true, false, true, false, "", false),
            P::Hidden
        );
    }
    ...
    fn refusal_live_tracks_the_deadline() {
        // No refusal armed ΓçÆ never live, regardless of clock.
        let now = Instant::now();
        assert!(!refusal_live(None, now));
        // Deadline still ahead ΓçÆ live.
        assert!(refusal_live(Some(now + Duration::from_millis(10)), now));
        // Deadline already passed ΓçÆ no longer live (strict `<`, so `now == t` also expires).
    ...
    fn tap_decision_immediate_when_not_speaking() {
        // Not speaking ΓçÆ act now regardless of any pending tap (dictation-start stays instant).
        let now = Instant::now();
    ...
    fn tap_decision_defers_then_skips_while_speaking() {
        let t0 = Instant::now();
        let w = Duration::from_millis(280);
        // Speaking, no prior tap ΓåÆ DEFER (first tap of a possible double).
        assert_eq!(tap_decision(true, None, t0, w), TapAction::Defer);
        // Second tap INSIDE the window ΓåÆ SKIP the current message.
        ...
        // Second tap BEYOND the window ΓåÆ DEFER again (a stale prior tap already fired as a
    ...
    fn caps_tap_action_is_pause_resume_in_both_modes() {
        // Dictation ON + ready: tap toggles recording (start pauses, stop resumes) ΓÇö never
        // clears.
        assert_eq!(
        ...
        // Dictation OFF: a tap PAUSES (held, not cleared/dropped), the next tap RESUMES ΓÇö
    ...
    fn caps_tap_action_refuses_on_but_not_ready_without_touching_the_voice() {
        ...
        // is REFUSED ΓÇö not a record toggle, and crucially NOT the off-mode voice pause,
        ...
        // Already recording: readiness is moot ΓÇö the tap still STOPS (the model was ready
    ...
    fn caps_tap_pauses_then_resumes_with_dictation_off() {
        ...
        // the voice (held ΓÇö so any narration that arrives stays queued, not dropped) and
        ...
        assert!(!d.is_recording(), "dictation off ΓåÆ the mic never opens");
        MockPlatform::tap(&mut d);
    ...
    fn refused_start_on_but_not_ready_never_pauses_the_voice() {
        ...
        // downloading/loading. Reaching that state needs a REAL `ttsq` ΓÇö
        ...
        // fresh `TtsManager` reports Parakeet not resident, so the tap is refused ΓÇö and a
        ...
        d.ttsq = Some(q.clone());
        MockPlatform::tap(&mut d);
        assert!(!d.is_recording(), "not ready ΓåÆ the mic must not open");
        assert!(
            !d.voice_paused,
        ...
        // Tap-happy user during the download: every further tap stays a pure refusal ΓÇö
    ...
    /// A controllable mock Platform for the ┬ºF long-press logic. All state is
    ...
    struct MockPlatform {
        ...
        /// Whether an editable field is "focused" ΓÇö backs `can_paste`, which
        ...
        /// of the default `Ok` ΓÇö lets tests exercise `refresh_caps_gate`'s live AX
        ...
        /// Queued transitions drained by `drain_caps_events` ΓÇö lets a test replay a
        ...
        /// Count of `type_text` (paste) calls ΓÇö lets the focus-check tests assert
        /// whether a confirm tap actually pasted.
        type_text_calls: Cell<u32>,
        /// Count of `press_enter` (auto-submit) calls ΓÇö lets the insert-only
        /// double-tap tests assert whether the paste also pressed Enter.
        press_enter_calls: Cell<u32>,
        /// Count of shared acquisition attempts / `release_caps_key` calls ΓÇö lets tests assert the
        ...
        /// order ΓÇö lets `reload_pushes_extra_paste_target_lists_to_the_platform` assert
        ...
        /// Every chord actually tapped, in order ΓÇö lets tests assert WHICH chord
        ...
        /// Simulated frontmost frontend app tag (`is_app_frontmost`).
        app_frontmost: RefCell<Option<String>>,
    }

    impl KeyInjector for MockPlatform {
        // A `tap_key` is one discrete press+release, so it bumps BOTH the down and up
        // counters the caps-state-machine tests assert on ΓÇö keeping every existing
    ...
    impl FrontmostWindow for MockPlatform {
        ...
        fn is_app_frontmost(&self, app_tag: &str) -> bool {
            self.app_frontmost.borrow().as_deref() == Some(app_tag)
        }
    ...
    fn mk(long_press_ms: u64) -> Engine<MockPlatform> {
        ...
        // Instant-based timer ΓÇö mirrors `listener.rs`'s `for_test` doing the same.
    ...
    impl MockPlatform {
        ...
        /// hold in between ΓÇö the gesture toggles dictation on the RELEASE. The LED is a
        ...
        /// the cancel), then release (a no-op ΓÇö the hold consumed the press). Requires a
        /// tiny `long_press_ms` so the sleep is short.
        fn hold(d: &mut Engine<MockPlatform>) {
            d.plat.caps_phys_down.set(true);
            d.tick(); // down edge
            std::thread::sleep(Duration::from_millis(12));
            d.tick(); // past threshold ΓåÆ cancel_all
            d.plat.caps_phys_down.set(false);
    ...
    /// deferred submit stops waiting ΓÇö tests would otherwise have to sleep out the
    ...
    fn dictation_preview_gates_the_landed_final_on_caps_held() {
        // Released, finalized transcript landed ΓåÆ surfaced + panel in confirm mode.
        assert_eq!(
        ...
        // long-press cancel) ΓÇö show the live partial, NOT in confirm mode (held always
        // wins, whatever the finalize state).
        assert_eq!(
            dictation_preview(&FinalState::Ready("hello world".into()), "hel", true),
            ("hel".to_string(), false)
        );
        // Idle (no final landed, not armed) ΓåÆ the live partial, not in confirm mode.
        assert_eq!(
        ...
        // true between the deposit and the tick's disarm ΓÇö the row that was only
    ...
    fn dictation_preview_armed_bridges_the_async_final_gap() {
        ...
        // still suppresses it ΓÇö `caps_held` wins over `Armed` too.
        assert_eq!(
    ...
    fn paste_buf_disarm_clears_armed_and_empty_but_leaves_ready() {
        ...
        p.disarm();
        assert_eq!(p.final_state, FinalState::Idle);
        // `Ready` is left alone ΓÇö the strictly-equivalent mapping of the old
    ...
    fn caps_held_mirrored_and_suppresses_finalized_transcript() {
        ...
        // state, and the preview must NOT flash the finalized text while held ΓÇö so the
    ...
    fn press_alone_does_not_start_or_light() {
        // THE FIX: a key-DOWN never starts recording and never lights the LED ΓÇö the
    ...
    fn tap_starts_dictation_on_release_and_lights() {
        ...
        // UP edge (a tap): recording starts and the LED lights ΓÇö ON RELEASE.
        d.plat.caps_phys_down.set(false);
        d.tick();
    ...
    fn second_tap_stops_and_extinguishes_on_release_not_press() {
        ...
        // RELEASE: stop + LED off ΓÇö the light extinguishes on release, not on press.
        d.plat.caps_phys_down.set(false);
        d.tick();
        ...
        // lands squarely in `Idle` ΓÇö never `ConfirmArmed`.
        assert!(
    ...
    fn long_press_cancel_from_idle_keeps_led_off_despite_os_latch_toggle() {
        ...
        // hold ΓÇö AFTER the press edge. A long-press cancel from idle must not leave the
        ...
        d.plat.caps_phys_down.set(true);
        d.tick(); // down edge ΓÇö idle, no light
        assert!(!d.plat.lock_state.get(), "no light on the press");

        // macOS toggles the OS caps-lock latch ON mid-hold.
        d.plat.lock_state.set(true);
        std::thread::sleep(Duration::from_millis(12));
        d.tick(); // past threshold ΓåÆ cancel_all; the held re-assert pins the LED OFF
    ...
    fn deferred_tap_while_speaking_still_syncs_the_led_on_start_and_stop() {
        ...
        // (`handle_tap` ΓåÆ `TapAction::Defer`) and only resolves later, from
        ...
        // reach `start_recording`/`stop_recording` with NO LED write at all ΓÇö only
        ...
        // doc) ΓÇö avoids the unrelated readiness gate that a `BuiltIn`/`test_stub`
        // combination would hit (the stub's fresh `TtsManager` reports Parakeet not
        // resident, refusing the tap before it ever reaches `start_recording` ΓÇö see
        ...
        // this is the check_pending_tap ΓåÆ toggle_dictation ΓåÆ start_recording path
        ...
        // recent pending tap, not recording state ΓÇö re-arm "speaking" since
        // `start_recording`'s `pause_for_record()` may have cleared it for real).
        q.set_active_for_test(true);
        MockPlatform::tap(&mut d);
        assert!(d.is_recording(), "stop tap deferred ΓÇö still recording");
        assert!(
            d.plat.lock_state.get(),
    ...
    fn event_driven_drains_a_sub_poll_tap_in_one_tick() {
        ...
        // real edges from one `tick()` call ΓÇö the exact case the polled sampler would
        ...
        assert!(
            d.is_recording(),
            "a down+up drained in one tick still starts dictation ΓÇö the tap isn't dropped"
        );
        assert!(
            d.plat.lock_state.get(),
        ...
        // recording ΓÇö same outcome as the polled two-tick `MockPlatform::tap`.
        let t1 = Instant::now();
    ...
    fn hold_while_recording_discards_and_release_does_not_re_toggle() {
        ...
        // Hold past the threshold ΓåÆ discard (abort the listen), LED off.
        d.plat.caps_phys_down.set(true);
        d.tick();
        std::thread::sleep(Duration::from_millis(12));
        d.tick();
        ...
        assert_eq!(
            d.plat.tap_down_calls.get(),
            starts + 1,
            "aborted the listen once (ClaudeNative abortΓåÆstop = one keypress)"
        );
    ...
    fn deferred_submit_pastes_when_async_final_lands() {
        ...
        MockPlatform::tap(&mut d); // stop ΓÇö defers
        ...
        // window has lapsed) tick ΓåÆ paste + submit.
    ...
    fn frontmost_terminal_is_a_paste_target_even_without_ax_focus() {
        ...
        // (`can_paste`). Terminals ΓÇö the app's main dictation target ΓÇö
        ...
        // Neither signal: genuinely nowhere to paste ΓåÆ glow on.
        d.plat.terminal_frontmost.set(false);
        d.tick();
        assert!(
            !d.paste.lock().unwrap().can_paste,
            "no editable field and no terminal ΓçÆ no paste target"
        );
    ...
    /// the double-tap window before returning ΓÇö the setup shared by the four
    /// lone-tap/double-tap ├ù default/inverted `double_tap_submit` tests below, so the
    ...
    fn arm_stop_tap(double_tap_submit: bool, second_tap: bool) -> Engine<MockPlatform> {
        ...
        MockPlatform::tap(&mut d); // start
        MockPlatform::tap(&mut d); // stop ΓÇö arms the deferred submit
        if second_tap {
            MockPlatform::tap(&mut d); // second tap inside the window ΓÇö flips the outcome
        }
        d
    }
    ...
    fn deferred_submit_waits_out_the_stop_double_tap_window() {
        ...
        assert_eq!(d.plat.press_enter_calls.get(), 1, "single tap ΓåÆ auto-Enter");
    }

    #[test]
    fn double_tap_on_stop_pastes_without_enter() {
        // The stop tap, then a SECOND tap inside the window ΓåÆ INSERT-ONLY: the
        ...
        // The old assert here ("insert-only consumed ΓÇö the next dictation submits
        // normally") checked `enter_after_paste` reset to its default; that reset is
        // now STRUCTURAL ΓÇö the flag lives inside `ConfirmArmed`, which the paste drops.
        assert!(
            matches!(d.gesture, GestureState::Idle),
    ...
    fn double_tap_submits_inverts_a_lone_tap_to_insert_only() {
        ...
        // insert-only ΓÇö the opposite of the default gesture. Also proves the fast-final
        ...
        assert_eq!(
            d.plat.press_enter_calls.get(),
            0,
            "lone tap ΓåÆ insert only when double_tap_submit is on"
        );
    }
    ...
    fn hold_after_stop_cancels_the_pending_paste() {
        // Stop, then a HOLD (long-press) before the final pastes ΓåÆ the universal
        // CANCEL discards the pending transcript instead of submitting it.
        let mut d = mk(10); // tiny long-press so the test hold is short
        d.plat.terminal_frontmost.set(true);
        d.stt = Box::new(DeferStt);
        MockPlatform::tap(&mut d); // start
        MockPlatform::tap(&mut d); // stop ΓÇö deferred submit armed
        d.paste.lock().unwrap().final_state = FinalState::Ready("discard me".into());
        MockPlatform::hold(&mut d); // long-press ΓåÆ cancel_all
        assert!(!d.is_confirm_armed(), "cancel disarmed the deferred submit");
        assert!(
            matches!(d.paste.lock().unwrap().final_state, FinalState::Idle),
            "cancel also clears the panel-visibility mirror ΓÇö the panel must not \
             stay up or reappear after a long-press cancel"
        );
        lapse_stop_window(&mut d); // no-op (cancel cleared the window) ΓÇö belt & braces
        d.tick();
        assert_eq!(
            d.plat.type_text_calls.get(),
            0,
            "cancel discarded the pending transcript ΓÇö nothing pasted"
        );
    ...
    fn double_tap_on_stop_consumes_the_second_tap_and_preserves_the_transcript() {
        // The second tap of the stop gesture must always be CONSUMED ΓÇö never falls
        // through to start a new recording ΓÇö regardless of what it does to the
        ...
        MockPlatform::tap(&mut d); // double tap (┬╡s later ΓÇö inside the window)
        assert!(
            !d.is_recording(),
            "the second tap did NOT start a new recording"
        );
    ...
    fn tap_after_the_window_starts_a_new_recording_and_drops_the_paste() {
        // A tap AFTER the double-tap window is a NEW dictation, not a late double ΓÇö
        ...
        d.plat.caps_phys_down.set(false);
        d.tick(); // release ΓåÆ a plain tap ΓåÆ new recording (wipes the pending transcript)
        assert!(
            d.is_recording(),
    ...
    // ΓöÇΓöÇ ┬ºE.4 Engine::reload over MockPlatform ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    fn reload_clears_state_aborts_inflight_and_drives_led_off() {
        ...
        // dictation (empty ladder) flips the resolved engine on EVERY platform ΓÇö unlike
        ...
        // This does not fabricate a spurious tap ΓÇö gesture detection is edge-based
    ...
    fn reload_caps_toggle_bumps_status_gate_only_on_real_transition() {
        ...
        // transition ΓÇö so the app's dot updates live across e.g. an Accessibility
    ...
    fn assemble_acquires_the_caps_key_only_when_starting_enabled() {
        // `mk`/`Engine::new` start with caps enabled (preflight Ok, default config) ΓÇö
    ...
    fn assemble_does_not_acquire_the_caps_key_when_starting_disabled() {
        ...
        // key suppression at all ΓÇö the bug this whole fix closes for the STARTUP path.
    ...
    fn reload_caps_toggle_acquires_and_releases_the_physical_key_on_real_transitions_only() {
        ...
        // re-acquire it ΓÇö but only on REAL transitions, matching the existing
        ...
        // A second OFF reload is a no-op transition ΓÇö must not release again.
        d.reload(&off_cfg);
        assert_eq!(
            d.plat.release_caps_key_calls.get(),
            1,
    ...
    fn caps_gate_off_clears_a_stale_in_flight_press_latch_and_forces_the_led_off() {
        ...
        // forced off here ΓÇö unlike `cancel_all`/the confirm-paste path, nothing else on
    ...
    fn tick_discards_a_queued_backlog_while_always_listen_mode_bypasses_the_gesture() {
        // Always-listen mode and `caps` are independent axes ΓÇö the platform can
    ...
    fn reload_engine_swap_mid_hold_clears_recording_icon() {
        ...
        // `stt_active` (the menu-bar recording icon) + the preview buffer ΓÇö not leave the
        ...
        // Disabling dictation (empty ladder) flips the resolved engine ΓåÆ stt_changed ΓåÆ rebuilt
    ...
    fn refresh_caps_gate_reacts_to_live_ax_trust_changes() {
        ...
        // branch ΓÇö an actual trust FLIP ΓÇö never ran. Drive both edges with the now-
        ...
        d.refresh_caps_gate();
        assert!(d.caps, "AX regranted ΓåÆ caps re-enabled live");
        assert!(
            caps_active.load(Ordering::Relaxed),
    ...
    fn reload_always_mode_lifecycle_builds_bypasses_caps_and_tears_down_on_exit() {
        ...
        // An in-flight PTT hold before the mode flips to Always must be torn down ΓÇö Always
        ...
        // PTT gesture machine entirely ΓÇö even with the physical key held.
        d.plat.caps_phys_down.set(true);
        d.tick();
        assert!(
            !d.caps_phys_prev,
    ...
    fn reload_always_mode_rebuilds_the_listener_on_a_live_stt_provider_switch() {
        // Finding #5's acceptance criterion ΓÇö "the selected STT provider is honored... in
        // always-listening mode" ΓÇö only held at CONSTRUCTION: `Listener::new` resolves its
        ...
        // case: Parakeet presence stays falseΓåÆfalse on both sides in this test host (no real
        // model files), same as it would stay trueΓåÆtrue on a real host where both the old
        ...
        // Provider preference only ΓÇö same engine selection, so this isolates
        ...
        // windows/linux, so it silently re-resolves to `OrtCpu` there ΓÇö caught on real macOS
        ...
        // other provider to switch to ΓÇö skip there rather than assert a transition that can't
    ...
    fn reload_claude_code_stt_falls_back_to_default_chord_without_injected_paths() {
        ...
        // instance's `paths` back to None and reloading again ΓÇö reload's STT-rebuild branch
