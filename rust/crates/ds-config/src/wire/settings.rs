...
mod tests {
    ...
    fn merge_preserves_unrelated_keys_and_cc_voice() {
        ...
        // The preference fields are unset (None) in `sample_voice()`, so they're
        // `skip_serializing_if`'d — absent from the wire object entirely.
    ...
    fn voice_preference_roundtrips_through_the_wire() {
        // Distinct from `sample_voice()` (which leaves the preference unset): exercise all
        // three preference states through the SAME IPC wire shape.
        let v = VoiceConfig {
    ...
    fn merge_emits_canonical_default_tokens() {
        ...
        // The preference fields default to unset (None) — absent from the wire object.
        assert!(merged["dontspeak"].get("tts_engine").is_none());
        assert!(merged["dontspeak"].get("stt_engine").is_none());
    }
