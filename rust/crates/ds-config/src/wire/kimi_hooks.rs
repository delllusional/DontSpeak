...
fn kimi_command(bin: &str, verb: &str, client: WiredAgent) -> String {
...
/// never touched.
pub fn merge_kimi_hooks(existing: &str, bin: &str, client: WiredAgent) -> Result<String, String> {
...
mod tests {
    ...
    fn merged(existing: &str) -> String {
        merge_kimi_hooks(existing, BIN, WiredAgent::KimiCode).expect("merge ok")
    }

    fn cmd(verb: &str) -> String {
        kimi_command(BIN, verb, WiredAgent::KimiCode)
    }
    ...
    fn expected() -> Vec<(String, String, i64)> {
        desired_entries(BIN, WiredAgent::KimiCode)
    }
    ...
    fn rewire_heals_a_changed_binary_path_by_replacing_stale_entries() {
        ...
        let second = merge_kimi_hooks(&first, new_bin, WiredAgent::KimiCode).expect("merge ok");
        ...
        assert_eq!(
            our_entries(&doc),
            desired_entries(new_bin, WiredAgent::KimiCode),
    ...
    fn unmergeable_scalar_hooks_errors() {
        let bad = "hooks = \"oops\"\n";
        let err = merge_kimi_hooks(bad, BIN, WiredAgent::KimiCode).unwrap_err();
