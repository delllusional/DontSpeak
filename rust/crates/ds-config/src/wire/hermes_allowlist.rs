...
pub fn desired_approvals(bin: &str, client: ds_client::WiredAgent) -> Vec<(String, String)> {
...
pub fn merge_hermes_allowlist(existing: &Value, bin: &str, client: ds_client::WiredAgent) -> Value {
...
mod tests {
    ...
    fn merge_into_empty_wires_all_event_command_pairs() {
        let out = merge_hermes_allowlist(&Value::Null, BIN, WiredAgent::Hermes);
        let arr = out["approvals"].as_array().unwrap();
        assert_eq!(arr.len(), 5);
        let desired = desired_approvals(BIN, WiredAgent::Hermes);
    ...
    fn merge_preserves_user_approvals_and_is_idempotent() {
        ...
        let once = merge_hermes_allowlist(&existing, BIN, WiredAgent::Hermes);
        ...
        let twice = merge_hermes_allowlist(&once, BIN, WiredAgent::Hermes);
    ...
    fn user_approval_containing_dontspeak_substring_is_not_misidentified() {
        ...
        let out = merge_hermes_allowlist(&existing, BIN, WiredAgent::Hermes);
        assert!(
            out["approvals"]
                .as_array()
                .unwrap()
    ...
    fn rewire_heals_stale_bin_path() {
        let first = merge_hermes_allowlist(&Value::Null, BIN, WiredAgent::Hermes);
        let new_bin = "/opt/dontspeak/bin/dontspeak";
        let second = merge_hermes_allowlist(&first, new_bin, WiredAgent::Hermes);
        ...
        assert_eq!(
            ...
            desired_approvals(new_bin, WiredAgent::Hermes).len()
        );
    }
    ...
    fn strip_removes_only_ours_and_prunes_empty() {
        ...
        let wired = merge_hermes_allowlist(&existing, BIN, WiredAgent::Hermes);
        ...
        let only_ours = merge_hermes_allowlist(&Value::Null, BIN, WiredAgent::Hermes);
    ...
    fn allowlist_pairs_lockstep_with_hook_commands() {
        assert_eq!(
            desired_approvals(BIN, WiredAgent::Hermes),
            super::super::hermes_hooks::desired_hook_commands(BIN, WiredAgent::Hermes)
        );
    }
}
