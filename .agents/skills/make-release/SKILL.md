...
3. Append `## Lines`: bare compare URL then the table from
   ...
   Table columns: Area / Code / Tests / Comments / **Size bump (avg bin)** — the last
   cell on Total is the mean size of the six published host packages (linux/macos/windows
   × aarch64/x86_64) minus the previous release’s mean (`gh release view` asset sizes;
   install scripts and checksums excluded). Needs both tags published (drafts OK).
