...
1. Resolves the right asset on the latest GitHub Release of `delllusional/DontSpeak`
   ...
   - Linux → `dontspeak-<version>-linux-<x86_64|aarch64>.tar.gz`
2. Verifies its SHA-256 against the release's `checksums.txt`.
3. Installs the app (macOS: unzip `DontSpeak.app` into `~/Applications` — per-user, no
   admin needed; Windows: extract to
...
Full uninstall is one command — the installer placed a standalone uninstaller that
removes the app, every client integration, and all data/models/caches:

- **macOS / Linux:** `~/.local/bin/dontspeak-uninstall`
  (or from a repo checkout: `scripts/uninstall.sh`)
- **Windows:** Settings › Apps › DontSpeak › Uninstall
  (or run `%LOCALAPPDATA%\Programs\DontSpeak\uninstall.ps1`)

To remove just the client integration and keep the app: `dontspeak wire --all --remove`.
