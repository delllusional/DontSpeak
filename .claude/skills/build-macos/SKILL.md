...
```
`bundle.sh` does the whole dev install: `install-daemon.sh` (engine + helper bins → `~/.local/bin`, stable-signed, places `dontspeak-uninstall`) → `dontspeak wire --all` (every registry client: Claude Code hooks + MCP, Desktop MCP, Codex hooks) → `build.sh` (Rust `release-ffi` staticlib + `swift build`) → icon compile → assemble + codesign **`~/Applications/DontSpeak.app`** (`DONTSPEAK_APP_DIR` overrides; the uninstaller honors the same). Release installs (`web/install.sh`) use the SAME per-user location — one layout, no `/Applications` copy to fight over the login item, the wire target, or TCC.
