...
- **Licensing** — does any new or updated dependency introduce linked GPL/LGPL code?
  ...
  manifest: a permissively licensed crate can `include_str!` data *generated by* a GPL tool
  (an espeak-ng-produced pronunciation dictionary, say), and `cargo deny` cannot see it
  because it reads the manifest, not `data/`. Generated output is not automatically GPL-covered;
  treat it as a provenance review and DontSpeak-policy question, not proof of a license violation.
