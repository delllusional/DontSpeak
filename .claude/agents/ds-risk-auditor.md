...
- **Licensing** — does any new or updated dependency introduce linked GPL/LGPL code?
  Cross-check `NOTICE.md` — external GPL tools (e.g. Speech Dispatcher's `spd-say`) must
  stay process-invoked, never linked. Also check the dependency's **payload**, not just its
  manifest — `cargo deny` reads only the manifest. Apply the bundled-GPL/provenance
  invariant in AGENTS.md (canonical statement in NOTICE.md's provenance section); a payload
  hit is a provenance-review question, not automatic proof of a license violation.
