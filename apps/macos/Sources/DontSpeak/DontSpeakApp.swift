...
private struct MenuBarLabel: View {
    ...
    var body: some View {
        // Capture `openWindow` for the AppKit reopen handler (see `WindowOpener`). This is a
        // SIDE EFFECT via `let _ =`, NOT a view modifier — the returned view stays a bare
        // `Image`, so the status item keeps hugging the glyph (see the layout note below). The
        // label is the one view instantiated at launch, so the action is ready before the first
        // Finder/Dock reopen.
...
private enum MenuBarIcon {
    /// Measured from the system mic pill.
    static let micPillAspect: CGFloat = 40.0 / 24.0
    /// Glyph box ÷ bar height (≈ the mic's prominence).
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// The user "opened" the already-running app again — double-clicked it in Finder /
    /// Applications, launched it from Launchpad or Spotlight, or clicked its Dock icon. An
    /// accessory app has no window to auto-reveal, so mirror Docker/Dropbox: show the status
    /// window — the SAME `Window(id: "main")` the tray menu's "Open" opens (via `WindowOpener`,
    /// which also brings the accessory app frontmost). `flag` is ignored: if a window is already
    /// visible SwiftUI just re-fronts it, which is the desired "click the icon → status" result.
