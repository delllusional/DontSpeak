...
enum EngineStatus: Equatable, Sendable {
    ...
    ///
    /// Routes through `word` — the SHARED Rust formatter (`ds_engine_state_word`) that returns ""
    /// for the ready states (running/idle) and the localized note otherwise. Its emptiness IS the
    /// note-vs-stats gate, the SAME one Windows and Linux use, so the decision can't drift per
    /// platform (this replaced a hand-kept per-state list here).
