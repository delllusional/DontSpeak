...
internal sealed class DictationPanel : IDisposable
{
    ...
    /// thread, read on the render thread; arrays are never mutated after publication. A `record`
    /// (not a plain class) so `with` expressions (see OnThemeChanged) get a compiler-generated
    /// field-for-field copy — a new field added here can't be silently dropped by a hand-written
    /// copy elsewhere.</summary>
    ...
    /// Pure data, no GPU access. `now` is injected (production passes <see cref="_clock"/>'s
    /// elapsed ms; <c>internal static</c> so <c>DontSpeak.WinUI.Tests</c> can drive it with fixed
    /// timestamps) rather than read from a live <see cref="Stopwatch"/>, so this needs no real
    /// <see cref="DictationPanel"/> instance (which would otherwise create a real Win32 window).</summary>
